...
DECLARE
  ...
  v_event_kind text;
BEGIN
  -- Validate substrate exists based on type
  CASE p_substrate_type
    WHEN 'block' THEN
      IF NOT EXISTS (SELECT 1 FROM context_blocks WHERE id = p_substrate_id) THEN
    ...
    WHEN 'reflection' THEN
      IF NOT EXISTS (SELECT 1 FROM reflections_cache WHERE id = p_substrate_id) THEN
